...
export const CanvasBackground: React.FunctionComponent<CanvasBackgroundProps> = (props) => {
  ...
  const handleHorizontalPositionChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setCanvasBackgroundValues({ ...canvasBackgroundValues, position: { ...canvasBackgroundValues.position, x: event.target.value } })
  };

  const handleVerticalPositionChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setCanvasBackgroundValues({ ...canvasBackgroundValues, position: { ...canvasBackgroundValues.position, y: event.target.value }, });
  };

  const handleBackgroundImageScale = (event: React.ChangeEvent<HTMLInputElement>) => {
    setCanvasBackgroundValues({ ...canvasBackgroundValues, size: event.target.value })
  }
  ...
  return <Fragment>
    <div className="bg-gray-medium d-flex justify-content-between align-items-center py-2 px-3">
      ...
      <input
        ...
        onChange={(e) => setCanvasBackgroundValues({ ...canvasBackgroundValues, color: e.target.value })}>
      </input>
    </div>
    <div className="list-group-item">
      ...
      <label htmlFor="backgroundHorizontalRange" className="form-label m-3">Horizontal Position ({canvasBackgroundValues.position.x} %)</label>
      <input
        type="range"
      ...
      <label htmlFor="backgroundVerticalRange" className="form-label m-3">Vertical Position ({canvasBackgroundValues.position.y} %)</label>
      <input
        type="range"
        ...
        className="form-range"></input>
      <label htmlFor="backgroundScaleRange" className="form-label m-3">Scale ({canvasBackgroundValues.size}%)</label>
      <input
        type="range"
        min={0}
        max={1000}
